...
function printDetailedResults(results: AnalyticsResult[]): void {
  ...
  for (const result of results) {
    ...
    console.log(
      `  Rate: ${formatPercent(result.successRate)} (${result.successes}/${result.iterations})`,
    );
    ...
    console.log(
      `  Repeated:       ${result.avgQuality.repeatedWords.toFixed(2)}`,
    );
    ...
    console.log(pc.dim('\nNew KPIs:'));
    console.log(
      `  Verse Distance: ${pc.magenta(result.avgQuality.verseDistance.toFixed(3))}`,
    );
    console.log(
      `  Line Balance:   ${result.avgQuality.lineLengthBalance.toFixed(3)}`,
    );
    console.log(
      `  Imagery:        ${result.avgQuality.imageryDensity.toFixed(3)}`,
    );
    console.log(
      `  Coherence:      ${result.avgQuality.semanticCoherence.toFixed(3)}`,
    );
    console.log(
      `  Verb Presence:  ${result.avgQuality.verbPresence.toFixed(3)}`,
    );
    ...
    console.log(
      `  verseDistance=${s.verseDistance}, lineBalance=${s.lineLengthBalance}, imagery=${s.imageryDensity}`,
    );
    console.log(
      `  coherence=${s.semanticCoherence}, verbPresence=${s.verbPresence}`,
    );
  }
}
...
function findOptimalConfig(results: AnalyticsResult[]): void {
  ...
  const scored = results
    .map((r) => ({
      result: r,
      composite:
        (r.successRate / 100) * 0.4 +
        Math.min(1, r.avgQuality.totalScore / 15) * 0.6,
    }))
  ...
  scored.forEach((s, i) => {
    ...
    console.log(
      `     Score: ${pc.cyan(s.composite.toFixed(3))} | Success: ${formatPercent(s.result.successRate)} | Quality: ${s.result.avgQuality.totalScore.toFixed(2)}`,
    );
  });
  ...
  console.log(
    `  Avg Quality:  ${best.result.avgQuality.totalScore.toFixed(2)}`,
  );
