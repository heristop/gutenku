...
const { stats } = storeToRefs(haikuStore);
const { globalStats, fetchGlobalStats, formatNumber } = useGlobalStats();

const haikusGenerated = computed(() => stats.value.haikusGenerated);
...
<template>
  ...
  >
    ...
    <div class="preview-card__body">
      <p class="preview-card__description">{{ t('home.tagline') }}</p>
...
.preview-card__illustration {
  ...
  view-transition-name: haiku-illustration;
  opacity: 0;
