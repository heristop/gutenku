...
function updatePosition() {
  ...
  left = Math.max(
    padding,
    Math.min(left, globalThis.innerWidth - tooltipRect.width - padding),
  );
  top = Math.max(
    padding,
    Math.min(top, globalThis.innerHeight - tooltipRect.height - padding),
  );
